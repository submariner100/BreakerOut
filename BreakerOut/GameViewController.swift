...
//  BreakerOut
//
//  Created by Macbook on 28/04/2016.
...
class GameViewController: UIViewController {
   ...
   // game.playSound(scnScene.rootNode, name: "Paddle.wav")
...
extension GameViewController: SCNPhysicsContactDelegate {
    ...
    func physicsWorld(world: SCNPhysicsWorld, didBeginContact contact: SCNPhysicsContact) {
        ...
        if contactNode.physicsBody?.contactTestBitMask == ColliderType.Barrier.rawValue {
        ...
        if contactNode.physicsBody?.categoryBitMask == ColliderType.Brick.rawValue {
            ...
            contactNode.runAction(SCNAction.waitForDurationThenRunBlock(120) {
                (node:SCNNode!) -> Void in node.hidden = false
                })
            
        }
        
        if contactNode.physicsBody?.contactTestBitMask == ColliderType.Paddle.rawValue {
